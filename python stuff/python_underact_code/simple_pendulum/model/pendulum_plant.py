...
class PendulumPlant:
    ...
    def forward_dynamics(self, state, tau):
        ...
        accn = (torque - self.m * self.g * self.l * np.sin(state[0])/2 -
                self.b * state[1] -
    ...
    def inverse_dynamics(self, state, accn):
        ...
        tau = accn * self.inertia + \
            self.m * self.g * self.l * np.sin(state[0])/2 + \
    ...
    def potential_energy(self, state):
        Epot = self.m*self.g*self.l*(1-np.cos(state[0]))/2
        return Epot

    def kinetic_energy(self, state):
        Ekin = (0.5*self.m*(self.l*state[1])**2.0)/3
        return Ekin
